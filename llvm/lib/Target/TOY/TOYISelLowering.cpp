...
const char *TOYTargetLowering::getTargetNodeName(unsigned Opcode) const {
  ...
  default:
    return NULL;
  case TOYISD::RET_FLAG: return "RetFlag";
  case TOYISD::LOAD_SYM: return "LOAD_SYM";
  case TOYISD::MOVEi32:  return "MOVEi32";
  case TOYISD::CALL:     return "CALL";
  }
}
...
TOYTargetLowering::TOYTargetLowering(const TargetMachine &TM,
  ...
  setStackPointerRegisterToSaveRestore(TOY::SP);
...
SDValue TOYTargetLowering::LowerGlobalAddress(SDValue Op, SelectionDAG& DAG) const
{
  EVT VT = Op.getValueType();
  GlobalAddressSDNode *GlobalAddr = cast<GlobalAddressSDNode>(Op.getNode());
  SDValue TargetAddr =
      DAG.getTargetGlobalAddress(GlobalAddr->getGlobal(), Op, MVT::i32);
  return DAG.getNode(TOYISD::LOAD_SYM, Op, VT, TargetAddr);
}
...
//  Chain =
  //    DAG.getCALLSEQ_START(Chain, DAG.getIntPtrConstant(NumBytes, dl), dl);
 ...
 // Mask = TRI->getCallPreservedMask(CallConv);
  ...
  //Chain = DAG.getCALLSEQ_END(Chain, DAG.getIntPtrConstant(NumBytes, dl),
    //                         DAG.getIntPtrConstant(0, dl), InFlag, dl);
  if (!Ins.empty()) {
    InFlag = Chain.getValue(1);
  }
...
SDValue TOYTargetLowering::LowerCallResult(
  ...
  CCInfo.AnalyzeCallResult(Ins, RetCC_TOY);
...
/// TOY formal arguments implementation
